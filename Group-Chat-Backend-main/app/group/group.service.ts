 ...
 */
export const getPublicGroups = async (user: IUserWithoutPassword,) => {
  return await Group.find({
    type: "public",
    members: { $nin: [user._id] }, // Exclude groups where the user is already a member
  });
};
...
export const createInvitation = async (
  ...
  return {
    frontendLink: `http://localhost:5173/invite-acccptence/${token}`,
    invitationLink: `http//localhost:3000/api/groups/accept-invitation/${token}`,
  };
};
