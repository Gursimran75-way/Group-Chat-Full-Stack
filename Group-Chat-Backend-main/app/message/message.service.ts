...
export const getAllMessages = async (
  ...
  if (!isGroup) throw createHttpError(400, "Group not found");

  return await Message.find({ groupId: data.groupId }).populate("senderId", "name email avatar").sort({ createdAt: 1 });
};
