...
export const getAllUser = asyncHandler(async (req: Request, res: Response) => {
  const result = await userService.getAllUser(req.params.id);
  res.send(createResponse(result));
});
...
export const getUserGroups = asyncHandler(
  async (req: Request, res: Response) => {
    if (req.user) {
      const result = await userService.getUserGroups(req.user);
      res.send(createResponse(result, "user groups fetched successfully"));
    } else {
      throw createHttpError(401, {
        message: "unauthorized",
      });
